...
import type { RailFreightFlow } from "@/lib/api";

interface RailFreightLayerProps {
  viewer: Viewer;
  enabled: boolean;
  flows: RailFreightFlow[];
  selectedFlow: number | null;
  /** Orchestrator clears its selection state when new flow data arrives. */
  onResetSelection: () => void;
  /** Shared refs owned by the orchestrator: the mouse-move tooltip
   *  handler reads `flowsRef` and `colorsRef`, and this component
   *  writes them at the end of every render. */
  flowsRef: RefObject<RailFreightFlow[]>;
  colorsRef: RefObject<string[]>;
  originalColorsRef: RefObject<Map<string, Color>>;
  originalWidthsRef: RefObject<Map<string, number>>;
  estimatedRef: RefObject<Map<string, boolean>>;
}

/**
 * Renders rail-freight corridor flows as a junction-graph traced over real
 * cities. Two internal effects:
 *
 * 1. **Render** (deps: flows, enabled): builds the city/junction graph,
 *    routes each flow along it using cardinal-spline interpolation, draws
 *    PolylineArrow (real data) or PolylineDash (estimated) entities tagged
 *    `rail_freight_<i>`. Stores original colour + width into shared refs
 *    so the highlight effect can restore them, and clears any prior
 *    selection by calling `onResetSelection`.
 *
 * 2. **Highlight** (deps: selectedFlow): when a flow is selected by the
 *    orchestrator's click handler, brighten + widen it and fade all
 *    other rail flows. When deselected, restore everything to original.
 *
 * Entity name convention `rail_freight_<i>` is what the orchestrator's
 * click handler and mouse-move tooltip handler pick on, so changing it
 * would silently break both.
 */
export function RailFreightLayer({
  viewer,
  enabled,
  flows,
  selectedFlow,
  ...
  useEffect(() => {
    ...
    originalWidthsRef.current.clear();
    ...
    const usFlows = flows.filter((rf) => flowRegion(rf.origin_iso, rf.destination_iso) === 'us');
    const euFlows = flows.filter((rf) => flowRegion(rf.origin_iso, rf.destination_iso) === 'eu');
    const asiaFlows = flows.filter((rf) => flowRegion(rf.origin_iso, rf.destination_iso) === 'asia');
    ...
    const validFlows = flows.filter((rf) => {
    ...
    validFlows.forEach((rf, vi) => {
      ...
      originalColorsRef.current.set(`rail_freight_${flowIdx}`, flowColor);
      originalWidthsRef.current.set(`rail_freight_${flowIdx}`, width);
      estimatedRef.current.set(`rail_freight_${flowIdx}`, rf.estimated);
      flowIdx++;
    });

    // Store flow data and colors for hover tooltips
    flowsRef.current = validFlows;
    colorsRef.current = flowColorsCss;
  ...
  useEffect(() => {
    viewer.entities.values.forEach((entity) => {
      ...
      const origColor = originalColorsRef.current.get(n);
      const origWidth = originalWidthsRef.current.get(n);
      const isEstimated = estimatedRef.current.get(n) ?? false;
